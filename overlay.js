    ...
    /**
    	...
    	jQuery("<your_tabs_container_selector>").overlay(optionalSettingsObject);
    	...
    	function Overlay (settings) {
    		...
    		**/
    		me.init = function () {
    			...
    			var $overlayShell = $('<div class="" id="' + me.id + '" role="dialog" aria-describedby="' + me.id + '-content" aria-label="' + appliedSettings.arialabel + '" data-widget="overlay container" data-name="' + appliedSettings.name + '"><div class="ibm-overlay-heading-con"><p class="ibm-icononly"><a href="#" class="ibm-close-link" role="button">Close</a></p></div><div id="' + me.id + '-content" class="content"></div></div>'),
    				$overlayContent,
    				didResize = false;
    		...
    		**/
    		...
    		function show () {
    			...
    			if ($content.innerHTML === "") {
    				console.warn("Will not show an empty overlay");
    				return false;
    			}
     
    			if (isShowing()) {
    				console.warn("This overlay is already showing");
    				return false;
    			}
